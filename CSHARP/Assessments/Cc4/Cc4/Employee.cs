...
namespace Cc4
{
    ...
    class Employee_details
    {
            ...
            List<Employee> empList = new List<Employee>
        {
            ...
            Console.WriteLine("Details of all employees:");//all employees details
            ...
            var notMumbaiEmployees = empList.Where(emp => emp.City != "Mumbai");//not mumbai locaion
            ...
            var assistantManagers = empList.Where(emp => emp.Title == "AsstManager");//assistant manager details
            Console.WriteLine("\nDetails of employees whose title is AsstManager:");
            foreach (var emp in assistantManagers)
            {
                Console.WriteLine($"Employee ID: {emp.EmployeeID}, Name: {emp.FirstName} {emp.LastName}, Title: {emp.Title}, DOB: {emp.DOB.ToShortDateString()}, DOJ: {emp.DOJ.ToShortDateString()}, City: {emp.City}");
            }
            var lastNameMustStartsWithS = empList.Where(emp => emp.LastName.StartsWith("S"));//Display Employees whose lastname starts with s
            Console.WriteLine("\nDetails of employees whose Last Name starts with S:");
            foreach (var emp in lastNameMustStartsWithS)
            {
                Console.WriteLine($"Employee ID: {emp.EmployeeID}, Name: {emp.FirstName} {emp.LastName}, Title: {emp.Title}, DOB: {emp.DOB.ToShortDateString()}, DOJ: {emp.DOJ.ToShortDateString()}, City: {emp.City}");
            }
            Console.ReadLine();
